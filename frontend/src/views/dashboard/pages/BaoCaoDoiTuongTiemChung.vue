  ...
  <div>
    ...
    >
      ...
      >
        ...
        >
          <v-icon left size="20"> mdi-filter-plus-outline </v-icon>
          Lọc danh sách
        </v-btn>
        <v-card-text v-if="showAdvanceSearch">
          <tim-kiem
            ref="timkiem"
            v-on:trigger-search="searchDangKyTiem"
            v-on:trigger-cancel="cancelSearchDangKyTiem"
          ></tim-kiem>
        </v-card-text>
        <v-card-text :class="breakpointName !== 'lg' ? 'px-0' : 'pt-0'">
          <div :class="breakpointName === 'xs' ? 'mb-3' : 'd-flex my-3'">
            ...
            <v-btn
              ...
              small
              class="mx-0"
              @click.stop="translateStatus('multiple')"
              :loading="processingAction"
              :disabled="processingAction"
            >
          ...
          >
            <template v-slot:header.muiTiemChung="{ header }">
              <div>
                <div class="py-2" style="border-bottom: 1px solid #dedede">
                  Thông tin mũi tiêm
                </div>
                <v-layout wrap>
                  <v-flex
            ...
            <template v-slot:item.hoVaTen="{ item, index }">
              <div
                :style="item.kiemTraTrung == 2 ? 'background: yellow' : ''"
                :title="item.kiemTraTrung == 2 ? 'Đăng ký trùng lặp' : ''"
              >
                <p class="mb-0" style="font-weight: 500">{{ item.hoVaTen }}</p>
                <p class="mb-2" style="color: blue">
            ...
            <template v-slot:item.diaChiNoiO="{ item, index }">
              <p class="mb-2">
                {{ item.diaChiNoiO }} - {{ item.phuongXaTen }} -
                {{ item.quanHuyenTen }} - {{ item.tinhThanhTen }}
              </p>
            </template>
            <template v-slot:item.muiTiemChung="{ item, index }">
              <div style="width: 250px; height: 100%">
                <v-layout
                  wrap
                  style="height: 100%"
                  v-if="item.muiTiemChung && item.muiTiemChung.length"
                >
                  <v-flex
                    class="xs12 md6"
                    style="border-right: 1px solid #dedede"
                    v-for="(item2, index2) in item.muiTiemChung"
                    :key="index2"
                  >
                    <p
                      class="py-2 mb-0"
                      v-if="
                        item.muiTiemChung &&
                        item.muiTiemChung[index2]['lanTiem'] == 1
                      "
                      style="text-align: left"
                    >
                      <span
                        >Ngày tiêm:
                        {{ item.muiTiemChung[index2]["ngayTiemChung"] }}</span
                      ><br />
                      <span
                        >Loại thuốc:
                        {{ item.muiTiemChung[index2]["loaiThuocTiem"] }}</span
                      ><br />
                      <span
                        >Địa điểm:
                        {{
                          item.muiTiemChung[index2]["diaDiemTiemChung"]
                        }}</span
                      >
                    </p>
                  </v-flex>
                  ...
                  >
                    <p
                      class="py-2 mb-0 pl-2"
                      v-if="
                        item.muiTiemChung &&
                        item.muiTiemChung[index2]['lanTiem'] == 2
                      "
                      style="text-align: left"
                    >
                      <span
            ...
            <template v-slot:item.action="{ item }">
              <div style="width: 150px">
                ...
                <v-tooltip
                  top
                  v-if="
                    userLogin['role_name'] == 'QuanTriHeThong' ||
                    userLogin['role_name'] == 'QuanTriCoSo' ||
                    userLogin['role_name'] == 'CanBoYTe'
                  "
                >
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn
                      @click="addMuiTiem(item)"
                      color="blue"
                      text
                      icon
                      class="ml-2"
      ...
      <v-dialog max-width="1000" v-model="dialogDetail" fullscreen>
        <v-card>
          <v-toolbar dark color="#0072bc">
          ...
          <v-card-text class="pt-0">
            <div class="my-3">
              ...
              <span style="color: #0072bc; font-weight: bold"
                >THÔNG TIN NGƯỜI TIÊM</span
              >
            </div>
            <v-layout wrap>
              <v-text-field
            ...
            <v-layout
              wrap
              v-if="
                detaiInfo['muiTiemChung'] && detaiInfo['muiTiemChung'].length
              "
            >
              <v-card
                outlined
                ...
                max-width="450"
                min-width="350"
                v-for="(item, index) in detaiInfo['muiTiemChung']"
                v-bind:key="index"
              >
                <div class="mb-2">
                ...
                <p class="mb-2">
                  Địa điểm tiêm: {{ item["diaDiemTiemChung"] }}
                </p>
              </v-card>
            </v-layout>
            ...
            <div class="my-3">
              ...
              <span style="color: #0072bc; font-weight: bold"
                >THÔNG TIN LỊCH HẸN</span
              >
            </div>
            <v-layout
              wrap
              v-if="
                detaiInfo['phieuHenTiem'] && detaiInfo['phieuHenTiem'].length
              "
            >
              <v-card
                outlined
                ...
                max-width="450"
                min-width="350"
                v-for="(item, index) in detaiInfo['phieuHenTiem']"
                v-bind:key="index"
              >
                <div class="mb-2">
                  <v-icon size="22" color="green" class="mr-3">
                ...
                <p class="mb-2">
                  Tình trạng:
                  {{ formatTinhTrangTiem(item["tinhTrangXacNhan"]) }}
                </p>
              </v-card>
            </v-layout>
      ...
      <v-dialog max-width="1000" v-model="dialogAddMuiTiem">
        <v-card>
          <v-toolbar dark color="#0072bc">
            <v-toolbar-title v-if="typeAction === 'add'"
              >Thêm thông tin tiêm chủng</v-toolbar-title
            >
            <v-toolbar-title v-else>Cập nhật thông tin</v-toolbar-title>
            <v-spacer></v-spacer>
            <v-toolbar-items>
              <v-btn icon dark @click="dialogAddMuiTiem = false">
                <v-icon>mdi-close</v-icon>
              </v-btn>
            </v-toolbar-items>
          </v-toolbar>
          <v-card-text class="mt-5">
            <v-form ref="formAddMuiTiem" v-model="validFormAdd" lazy-validation>
              <v-layout wrap>
                <v-text-field
...
export default {
  ...
  created() {
    ...
    if (!isSigned) {
      vm.$router.push({
        path: "/login?redirect=/pages/danh-sach-dang-ky-tiem-moi",
      });
      return;
    }
  ...
  methods: {
    ...
    getDanhSachDangKyChinhThuc(pageIn, dataSearch) {
      ...
      let filter = {
        ...
        tinhtrangdangky: 1,
        cmtcccd:
          dataSearch && dataSearch["CMTCCCD"] ? dataSearch["CMTCCCD"] : "",
        nhomdoituong:
          dataSearch && dataSearch["NhomDoiTuong"]
            ? dataSearch["NhomDoiTuong"]
            : "",
        ngaydangki:
          dataSearch && dataSearch["NgayDangKi"]
            ? dataSearch["NgayDangKi"]
            : "",
        hovaten:
          dataSearch && dataSearch["HoVaTen"] ? dataSearch["HoVaTen"] : "",
        diabancosoid:
          dataSearch && dataSearch.hasOwnProperty("DiaBanCoSo_ID")
            ? dataSearch["DiaBanCoSo_ID"]
            : "",
        cosoytema:
          dataSearch && dataSearch["CoSoYTe_Ma"]
            ? dataSearch["CoSoYTe_Ma"]
            : "",
        kiemtratrung:
          dataSearch && dataSearch["KiemTraTrung"]
            ? dataSearch["KiemTraTrung"]
            : "",
        tinhthanhma:
          dataSearch && dataSearch["TinhThanh_Ma"]
            ? dataSearch["TinhThanh_Ma"]
            : "",
        quanhuyenma:
          dataSearch && dataSearch["QuanHuyen_Ma"]
            ? dataSearch["QuanHuyen_Ma"]
            : "",
        phuongxama:
          dataSearch && dataSearch["PhuongXa_Ma"]
            ? dataSearch["PhuongXa_Ma"]
            : "",
        tinhthanhten: "",
        quanhuyenten: "",
        phuongxaten: "",
    ...
    exportDanhSach() {
      ...
      let filter = {
        ...
        data: {
          ...
          tinhtrangdangky: 1,
          cmtcccd:
            vm.dataInputSearch && vm.dataInputSearch["CMTCCCD"]
              ? vm.dataInputSearch["CMTCCCD"]
              : "",
          nhomdoituong:
            vm.dataInputSearch && vm.dataInputSearch["NhomDoiTuong"]
              ? vm.dataInputSearch["NhomDoiTuong"]
              : "",
          ngaydangki:
            vm.dataInputSearch && vm.dataInputSearch["NgayDangKi"]
              ? vm.dataInputSearch["NgayDangKi"]
              : "",
          hovaten:
            vm.dataInputSearch && vm.dataInputSearch["HoVaTen"]
              ? vm.dataInputSearch["HoVaTen"]
              : "",
          diabancosoid:
            vm.dataInputSearch &&
            vm.dataInputSearch.hasOwnProperty("DiaBanCoSo_ID")
              ? vm.dataInputSearch["DiaBanCoSo_ID"]
              : "",
          cosoytema:
            vm.dataInputSearch && vm.dataInputSearch["CoSoYTe_Ma"]
              ? vm.dataInputSearch["CoSoYTe_Ma"]
              : "",
          kiemtratrung:
            vm.dataInputSearch && vm.dataInputSearch["KiemTraTrung"]
              ? vm.dataInputSearch["KiemTraTrung"]
              : -1,
          tinhthanhma:
            vm.dataInputSearch && vm.dataInputSearch["TinhThanh_Ma"]
              ? vm.dataInputSearch["TinhThanh_Ma"]
              : "",
          quanhuyenma:
            vm.dataInputSearch && vm.dataInputSearch["QuanHuyen_Ma"]
              ? vm.dataInputSearch["QuanHuyen_Ma"]
              : "",
          phuongxama:
            vm.dataInputSearch && vm.dataInputSearch["PhuongXa_Ma"]
              ? vm.dataInputSearch["PhuongXa_Ma"]
              : "",
          tinhthanhten: "",
          quanhuyenten: "",
          phuongxaten: "",
      ...
      vm.$store
        .dispatch("exportDanhSach", filter)
        .then(function (result) {
    ...
    translateStatus(item) {
      ...
      if (x) {
        vm.$store
          .dispatch("updateRegistrationStatus", filter)
          .then(function (result) {
            vm.$store.commit("SHOW_SNACKBAR", {
              show: true,
